...
- Local dev: Angular uses same-origin API URLs (`Client/src/app/core/api/api.config.ts`); `Client/proxy.conf.json` proxies `/api/**` to `http://localhost:8080`. Production Firebase Hosting (`firebase.json`, project `honest-car-498923`) rewrites `/api/**` to Cloud Run `honest-car-server` in `us-central1` (`run.serviceId` is the service name, not the project ID). Custom domain `honest-car.co` is connected in Firebase Console (Hosting → Custom domains). No server CORS config. Deploy frontend: `pnpm --dir Client deploy:hosting`.
