...
export default function Maintenance({ chosenVehicle, user }) {
  ...
  const handleReminderConfirm = (item) => {
    ...
    setReminderDialogOpen((prevState) => ({
      ...prevState,
  ...
  useEffect(() => {
    const fetchAndConvertMaintenance = async () => {
      ...
      try {
        const response = await fetch(
          `http://localhost:8080/api/external/find_maintenance/${chosenVehicle.vin}/${chosenVehicle.mileage}`,
    ...
    const updateMaintenanceRecord = async (mappedExternalApiMaintenance) => {
      try {
        const response = await fetch(
          "http://localhost:8080/api/maintenance/update_maintenance_records",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
    ...
    const fetchMaintenanceById = async () => {
      ...
      try {
        const response = await fetch(
          `http://localhost:8080/api/maintenance/${chosenVehicle.vinId}`,
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
    ...
    const operations = async () => {
      ...
      const completedMaintenanceTest = fetchedMaintenanceById.filter(
        (record) => record.dateCompleted !== null
      );
      setCompletedMaintenance(completedMaintenanceTest);
      const upcomingMaintenanceTest = fetchedMaintenanceById.filter(
        (record) => record.dateCompleted === null
      );
                ...
                <>
                  <IconButton
                    ...
                    onClick={() => {
                      ...
                      todayMaintenanceItem.maintenanceRecordId =
                  ...
                  <Dialog
                    open={reminderDialogOpen[item.description]}
                  ...
                  >
                    ...
                    <DialogActions>
                      <Button
                        onClick={() =>
                          handleReminderDialogClose(item.description)
                        }
                        color="primary"
                      >
                        Cancel
                      ...
                      <Button
                        onClick={() => handleReminderConfirm(item)}
                        color="primary"
                      >
                        Confirm
                      </Button>
