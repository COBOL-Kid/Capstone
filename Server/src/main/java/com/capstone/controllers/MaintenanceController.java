...
public class MaintenanceController {
  ...
  public ResponseEntity<?> getCompletedMaintenance(
    ...
    List<CompletedMaintenanceResponse> completedMaintenance =
        maintenanceTrackingService.findCompletedMaintenance(user, vin);
  ...
  public ResponseEntity<?> getUpcomingMaintenance(
    ...
    List<UpcomingMaintenanceResponse> upcomingMaintenance =
        maintenanceTrackingService.findUpcomingMaintenance(user, vin);
  ...
  public ResponseEntity<?> getMaintenanceCosts(
      @AuthenticationPrincipal User user,
      @PathVariable("vin")
          @Pattern(
              regexp = VIN_PATTERN,
    ...
    List<MaintenanceCostResponse> costs =
        maintenanceTrackingService.findMaintenanceCosts(user, vin);
    if (costs.isEmpty()) {
      return new ResponseEntity<>(HttpStatus.NO_CONTENT);
    }
  ...
  public ResponseEntity<?> completeMaintenance(
      @AuthenticationPrincipal User user, @Valid @RequestBody CompleteMaintenanceRequest request) {
    if (user == null) {
      return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);
    }
    return new ResponseEntity<>(
        maintenanceTrackingService.completeMaintenance(user, request), HttpStatus.CREATED);
  }
  ...
  public ResponseEntity<?> uncompleteMaintenance(
    ...
    try {
      maintenanceTrackingService.uncompleteMaintenance(user, completedMaintenanceId);
      return new ResponseEntity<>(HttpStatus.NO_CONTENT);
