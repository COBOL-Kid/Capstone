...
public class VehicleDataMapper {
  ...
  private static final DateTimeFormatter RECALL_DATE_FORMAT =
  ...
  public List<MaintMileage> toMaintMileages(
      VehicleType vehicleType, MaintenanceScheduleResponse maintenanceScheduleResponse) {
    List<MaintMileage> maintMileages = new ArrayList<>();
    if (maintenanceScheduleResponse == null
        || maintenanceScheduleResponse.data() == null
        || maintenanceScheduleResponse.data().maintenance() == null) {
      return maintMileages;
    }
    for (MaintenanceScheduleResponse.MaintenanceInterval interval :
        maintenanceScheduleResponse.data().maintenance()) {
      if (interval == null
          || interval.mileage() == null
          || interval.mileage().miles() == null
  ...
  public List<MaintCost> toMaintCosts(
    ...
    for (RecallProviderResponse.RecallItem item : recallResponse.data()) {
      ...
      Recall recall =
          new Recall(
              vehicleType,
              nhtsaCampaignNumber,
              parseRecallDate(item.reportReceivedDate()),
              requiredText(item.component()),
              requiredText(item.summary()),
              requiredText(item.consequence()),
