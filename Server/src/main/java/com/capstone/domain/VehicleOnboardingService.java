...
public class VehicleOnboardingService {
    ...
    public AddVinResponse addVinToUser(User user, AddVinRequest request) {
        ...
        int currentMileage = request != null ? request.currentMileage() : 0;
        if (currentMileage < 0) {
    ...
    private SupplementalVehicleData fetchSupplementalVehicleData(String normalizedVin) {
        try (var executor = Executors.newVirtualThreadPerTaskExecutor()) {
            Future<OwnerManualResponse> ownerManual = executor.submit(
                    () -> vehicleDataProviderClient.getOwnerManual(normalizedVin));
            Future<MaintenanceScheduleResponse> maintenanceSchedule = executor.submit(
                    () -> vehicleDataProviderClient.getMaintenanceSchedule(normalizedVin));
            Future<RepairCostResponse> repairCosts = executor.submit(
                    () -> vehicleDataProviderClient.getRepairCosts(normalizedVin));
