...
class RequestValidationTest {
  ...
  void shouldValidateAuthenticationRequests() {
    assertTrue(
        validator.validate(new AuthenticationRequest("driver@example.com", "secret")).isEmpty());
  ...
  void shouldValidateRegistrationRequests() {
    assertTrue(
        validator
            .validate(new RegisterRequest("Pat", "Driver", "driver@example.com", "LongSecret1!"))
            .isEmpty());
        ...
        validator
            .validate(
                new CompleteMaintenanceRequest(
                    "JTENU5JR6M5962554", 11L, LocalDate.now(), 45100, 120.50, "Dealer service"))
            .isEmpty());
